...
class PyAction():
    ...
    def get_displayinfo(self):
        ...
        for i in range(-3,4):
            ...
            for j in range(-3,4):
                if(i==0 and j==0):
                    self.input[i*7+21+j+3]=float(-2.0)
    ...
    def blockcollision_y(self):
        ...
        for block in self.blocks:
            ...
            if not(blotrx <= myblx or mytrx <= bloblx or blobly <= mytry or blotry >= mybly) :
                ...
                self.on_floor=False
        """
        if(self.fpvy>0.0 and (bpoint[mybly//GS][(mytrx-5)//GS]==True or bpoint[mybly//GS][(myblx+5)//GS]==True)):
            self.fpy=(mybly//GS)*GS-GS
            epoint[int(self.fpy)//GS][int(self.fpx)//GS]=1
            ...
            self.dive=False
            return
        elif(self.fpvy<0.0 and (bpoint[mytry//GS][(mytrx-5)//GS]==True or bpoint[mytry//GS][(myblx+5)//GS]==True)):
            self.fpy=(mytry//GS)*GS+GS
            epoint[int(self.fpy)//GS][int(self.fpx)//GS]=1
            self.fpvy=-self.initvy
            return
