...
class PyAction():
    GS=32
    def __init__(self,t,mapname,networkname):
        ...
        self.metainput=np.zeros(16,dtype=np.float)
        self.controll=0
        prev_action=0
        prev_inputs = []
        ...
        actor=neat.nn.FeedForwardNetwork.create(EnemyNet,actorconfig)
        #multiactor=neat.nn.FeedForwardNetwork.create(MultiNet,config)
        print('Are you Ready?')
        sleep(5)
        while True:
            """
            if(checkact%20==0):
                metainputs=self.get_metainfo()
                choice=np.argmax(multiactor.activate(metainputs))
                if(choice==0):
                    actor=neat.nn.FeedForwardNetwork.create(NoNets[choice],actorconfig)
                elif(choice>=1 and choice<=15):
                    print("Now Playing Enemy"+str(choice))
                    actor=neat.nn.FeedForwardNetwork.create(EnemyNets[choice-1],actorconfig)
                else:
                    choice-=15
                    print("Now Playing Jump" + str(choice))
                    actor=neat.nn.FeedForwardNetwork.create(JumpNets[choice-1],actorconfig)
            if(checkact%frameskip==0):
                inputs = self.get_displayinfo()
                #print(inputs)
                prev_inputs=inputs
                ...
                action = prev_action
            checkact+=1
            Python.controll = action
            """
            if(checkact%frameskip==0):
                self.input = self.get_displayinfo()
                #print(self.input)
                prev_inputs=self.input
                action = np.argmax(actor.activate(self.input))
                prev_action=action
            else:
                inputs=prev_inputs
                action = prev_action
    ...
    def get_displayinfo(self):
        ...
        #print(input)
        self.input[49]=self.map.python.fpvx/5.0
        self.input[50]=self.map.python.fpvy/10.0
        self.input[51]=self.map.python.state
        ...
        if(self.map.python.dive==True):
            ...
            self.input[53]=0.0
        self.input[54]=self.map.python.HP/100.0
        if(self.map.python.rejump_timer==0 and self.map.python.state==0):
            self.input[55]=1.0
        else:
            self.input[55]=0.0
        if(self.map.python.redattack_timer==0):
            self.input[56]=1.0
        else:
            self.input[56]=0.0
        ...
        for i in range(-7,8):
            ...
            for j in range(-7,8):
                ...
                if spoint[checky][checkx]==1:
                    spikecount+=1
        #print(input)
        self.metainput[0]=blockcount/(14*14)
        self.metainput[1]=enemycount/(14*14)
        self.metainput[2]=spikecount/(14*14)
        self.metainput[3]=itemcount/(14*14)
        self.metainput[4]=self.map.python.HP/100.0
        self.metainput[5]=self.map.python.fpvx/5.0
        self.metainput[6]=self.map.python.fpvy/10.0
        self.metainput[7]=self.map.python.state
        if(self.map.python.on_floor==True):
            self.metainput[8]=1.0
        else:
            self.metainput[8]=0.0
        if(self.map.python.dive==True):
            self.metainput[9]=1.0
        else:
            self.metainput[9]=0.0
        if(self.map.python.rejump_timer==0 and self.map.python.state==0):
            self.metainput[10]=1.0
        else:
            self.metainput[10]=0.0
        if(self.map.python.redattack_timer==0):
            self.metainput[11]=1.0
        else:
            self.metainput[11]=0.0
        self.metainput[12]=self.map.python.fpx/self.map.width
        self.metainput[13]=self.map.python.fpy/self.map.height
        self.metainput[14]=self.map.python.score/10000.0
        self.metainput[15]=self.gametime/1840.0
        
        return self.metainput
...
class Python(pygame.sprite.Sprite):
    ...
    resistlist=[0.8,1.0]
    gliplist=[1.0,0.5]
    controll = [-10.0, -10.0, -10.0, -10.0, -10.0, -10.0]
    left_images={}
    right_images={}
    global Crushpoint
...
def parser():
    usage='Usage: python {} --network [NETWORKNAME] --stage [STAGENAME.map]'.format(__file__)
    arguments=sys.argv
    stage_position=""
    if len(arguments)!=5:
        print(usage)
    ...
    if '-n' in options or '--network' in options:
        network_position=arguments.index('-s')\
                        if '-n' in options else arguments.index('--network')
        network_file=arguments[network_position+1]
        network_file="Network/"+network_file
        PyAction(0,stage_file,network_file)
