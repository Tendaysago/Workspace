...
def eval_genome(genome, config,stage):
    ...
    trystage="data/jump"+str(stage)+".map"
    fitness=0.0
    for i in range(1):
...
def eval_genomes(genomes, config):
    ...
    if(nowstageindex>=1):
        ...
        traintime=70
    if(nowstageindex>=1):
        print("Now try jump stage No."+str(stage_list[nowstageindex]))
    else:
        print("Now try jump warmup stage")
    clearnum=0
    for genome_id,genome in genomes:
        genome.fitness=eval_genome(genome,config,stage_list[nowstageindex])
    if(clearnum>=45 or nowtry==traintime):
        if(nowstageindex<1):
            if(clearnum>=45):
            ...
            if(clearnum>=45):
                print("Jump stage No."+str(stage_list[nowstageindex])+"Clear ! Next Jump stage No."+str(stage_list[nowstageindex+1]) )
            else:
                print("Jump stage No."+str(stage_list[nowstageindex])+"Give Up. Next Jump stage No."+str(stage_list[nowstageindex+1]) )
            nowstageindex+=1
            firstclearnum=-1
            firstclear=False
        else:
            ...
            if(clearnum>=45):
                MasterClear=True
                print("Jump stage No."+str(stage_list[nowstageindex])+" Clear ! Next generation is last Episode." )
            else:
                print("Jump stage No."+str(stage_list[nowstageindex])+" Give Up. Next generation is last Episode.")
    print(nowstageindex)
    print(nowtry,clearnum)
    print(clearinfo)
    nowtry+=1

def run(stage_number):
    ...
    global Getnet
    stage_list=[0]
    stage_list.append(int(stage_number))
    print(stage_list)
    ...
    # Save the winner.
    if(Getnet==False and MasterClear==True):
        with open('Network/winner-jumpNo'+stage_number, 'wb') as f:
            pickle.dump(winner, f)
        Getnet=True

    print(winner)
    print(clearinfo)
    print('Jump Stage No.'+stage_number+' '+str(learningtime)+' learning Finish!')
    #print('Jump Stage No.'+file_number+' multiple learning Finish!')
...
def parser():
    ...
    if '-n' in options or '--num' in options:
        ...
        for data in clearinfo:
            if(data[0]!=0 and not(data[1]==-1 and data[2]!=-1)):
                print(data)
