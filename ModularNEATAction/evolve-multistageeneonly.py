    ...
    for i in range(1):
        ...
        if(clear==True):
            ...
            if(trainend==False):
                ...
                with open('Results/multienemyonly'+str(stage)+'.txt','a') as f:
...
def run(file_number):
    ...
    actorconfig = neat.Config(neat.DefaultGenome, neat.DefaultReproduction,
                         ...
                         actorconfig_path)
    config_path=os.path.join(local_dir, 'config-action-multi-only')
    ...
    if(trainend==False):
        ...
        with open('Results/multienemyonly'+str(file_number)+'.txt','a') as f:
    ...
    #print(winner)
    print("Now Multi Stage with Enemy only Network version Clear info is ")
    print(clearinfo)
